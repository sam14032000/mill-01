...
**The trade, stated because founders will feel it:** the PM and engineer are now stricter line by line. Anything added to a spec without a named user, a job and a metric will be excluded and listed rather than quietly written in. That is the bar working, but it means a working session accumulates named refusals — which is the point, and is also the thing to revisit if it becomes noise.
