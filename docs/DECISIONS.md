...
**Not yet built, flagged rather than silently skipped:** Change 4's "one suggestion before proto" (audit offered once, ignorable, when a project first enters proto mode) — the only prompt to audit anywhere in the system now that audit is entered rather than triggered. Tracked as follow-up work, not deferred indefinitely: without it an idea can go brainstorm → proto with nothing ever asking whether it should die.
