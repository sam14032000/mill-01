...
- **Broad, founder-pushed (`find` `mode:"broad"`, and every slash / `@Mill find`).** When the founder explicitly asks to look something up or dig in: **5 sub-queries × 8 results** — breadth, not depth (`search_depth` stays `"basic"`, never a research pass). In a **project**, the full summary + query list + every scanned source is written to `ideas/<id>/find/<stamp>.md`, committed, and attached to the thread (`files.uploadV2` — needs the `files:write` scope; degrades to "saved to `<path>`" without it); Slack gets a **rundown that fits its block limit** plus the attachment, not the whole report. A one-line entry is appended to `ideas/<id>/find/index.md`, which `readOriginContext` loads into every thread of the project — so a report run in Brainstorm is referenceable from Research, Prototype, etc. It is still surface search, not evidence: the file header, the index, and the context block all say so, and `find/` is a **separate directory from `docs/`** precisely so it never reaches `/test`'s `DOC_PATH` or the audit as a document. In a bare `#chats` (no project) there's nowhere to store a report, so `mode:"broad"` there posts the old capped inline message.
