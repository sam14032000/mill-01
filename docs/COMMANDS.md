...
**Two modes (D-53).** Any founder invocation — `/find`, `@Mill find`, or the agent's `find(mode:"broad")` because you asked it to dig in — runs **broad**: 5 sub-queries, 8 results each (breadth, not depth). In a project, the full report is written to `ideas/<id>/find/<stamp>.md`, committed, attached to the thread, and indexed in `ideas/<id>/find/index.md` — which every thread of the project sees (referenceable from anywhere in the project); Slack gets a rundown that fits its limit plus the attachment. `find/` is a separate directory from `docs/` so a surface search never reaches `/test` or the audit as a document. In a bare `#chats` it's a capped inline message. Attachments need the `files:write` scope; without it the report is still in the repo and the rundown names the path.

Separately, the agent may call `find(mode:"quick")` **on its own**, mid-reply, when it needs one concrete missing fact to answer well (a price, a rule, a market number) — 1–2 queries, folded into the prose reply with the `_(quick web check — not verified, not evidence)_` marker, no separate block, no file. It is told to do this only for a missing fact, never for general uncertainty. `search_initiated_by` (`agent`/`founder`) is logged on every search.
