...
**Per-key daily budgets (LiteLLM virtual keys, C-05).** `mill-flash` **$1.00/day**, `mill-research` $3.00/day, `mill-audit` $2.00/day, all `budget_duration: "1d"`. `mill-flash` ran at $0.50/day through the D-51 build; **D-53 put a `flash-fast` agent-loop call on every conversational turn** (previously only slash commands and brainstorm hit `flash-fast`), the observed peak reached $0.50/day and tripped the cap, so it was raised to $1.00 via `/key/update` (key value preserved; `docs/build-guide.md` Part 7.3 updated same commit). Raise again only against a fresh peak, not pre-emptively.
