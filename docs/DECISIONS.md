...
- **Broad, founder-pushed (`find` `mode:"broad"`, and every slash / `@Mill find`).** When the founder explicitly asks to look something up or dig in: more distinct sub-queries (up to 5) and more results per query (8) — **breadth, not depth** (`search_depth` stays `"basic"`, never a research pass). Posted as the visibly-separate `*🔎 Surface search (broad):*` block with the not-evidence footer.
