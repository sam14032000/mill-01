...
`save` is now an action and a tool (project-only, no arguments, writes the current chat's mode document). Asking for it runs it, the same way "attack this" runs `/attack`. With that, automatic writing is worth removing rather than merely redundant: it meant a document could change without the founder asking and without them being able to point at what caused it. Both automatic paths are gone — the every-6-turns fold and the on-mode-switch upstream sync.

**What is NOT removed:** the missing-input and stale-input offers. Those are button-gated, so a founder still approves each write, which is the D-30 principle the original entry invoked.
