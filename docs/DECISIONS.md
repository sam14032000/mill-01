...
**Enforcement added (1 September 2026), because the rule was recorded and then violated.** *"`max_tokens` is at least 4096 everywhere reasoning is enabled"* was written here and then broken at eight call sites — the persona refusal check ran at 700, `commands/find.js` planned queries at 256. It cost a founder a working turn: the PM's refusal spent 675 of its 700 tokens reasoning and was cut off mid-sentence, before the `UNBLOCK:` line its own contract requires, so what reached the thread was an accusation with no way forward. The same call at 4096 produced a complete, specific refusal. A convention that has to be remembered at every call site is not a guard, so `llm.js` now floors `max_tokens` centrally (`MILL_MIN_MAX_TOKENS`, default 4096) and warns once per site when it raises one. Raising a cap costs nothing when the answer is short — billing is per token generated, not per token allowed — while a cap set too low fails silently, and that asymmetry is what decides it.
...
**Per-key daily budgets (LiteLLM virtual keys, C-05).** `mill-flash` **$2.00/day**, `mill-research` $3.00/day, `mill-audit` $2.00/day (Fable **5.1** since 9 Sep 2026, D-59), **`mill-code` $4.00/day** (proto's coding agent, D-58), all `budget_duration: "1d"`. `mill-flash` ran at $0.50/day through the D-51 build; **D-53 put a `flash-fast` agent-loop call on every conversational turn** (previously only slash commands and brainstorm hit `flash-fast`), the observed peak reached $0.50/day and tripped the cap, so it was raised to $1.00 via `/key/update` (key value preserved; `docs/build-guide.md` Part 7.3 updated same commit). **Raised to $2.00/day on 2 September 2026** — founders' call, same mechanism, key value preserved again.
...
**The operational lesson, which is the durable part.** A per-key daily cap is a runaway guard, and on a single-key-per-tier system it is also a single point of failure for the founders' whole day. Verification work that makes real model calls should be metered against the same cap it can exhaust — check spend between test files and stop at a guard rail — rather than run to completion and discover the cost afterwards. Raise the cap against a fresh peak, not pre-emptively, and say plainly whose traffic produced the peak.

**`mill-code` raised $2.00 → $4.00/day (9 September 2026).** Founders' call, on a **fresh peak from real use** — unlike the `mill-flash` raise a week earlier, which was agent testing and is labelled as such above. A plan → adjust → adjust → build cycle at `effort: high` spent $2.169 in one working session, and the build failed *after* the founder had approved the plan, losing it.

Two things changed with the raise so the peak should not recur at that level: the specs moved out of `--append-system-prompt` and onto disk, cutting the per-turn brief from ~8,900 tokens to ~218 (D-58); and a pre-flight budget check now refuses before spawning, so an approved plan is never burned by a cap the founder could have been warned about.

**State the arithmetic plainly rather than leaving it implied.** $4/day is $120/month if drawn down daily — more than D-25's entire $100 all-in ceiling. It is a runaway *guard*, not a budget: real days will sit far below it, and the runbook's line for prototype work is $12/month. If observed `mill-code` spend approaches even half the cap on ordinary days, that is the signal that proto has stopped being validation and become development — which D-29 already says is a different conversation with a different budget, and should be treated as that conversation rather than another raise.
...
**Two research modes on the agent loop.**
- **Inline fact check, agent-initiated (`find` `mode:"quick"`).** The agent calls it *mid-reply* when answering well needs one concrete missing fact — a price, a rule/threshold, a market number, a date, whether a named company exists. 1–2 Tavily queries (`search.js` `inlineFactSearch`), returned as a string the model folds into its prose; **no separate Slack block**; `agent.js` guarantees the marker `_(quick web check — not verified, not evidence)_` on the reply. The trigger is deliberately narrow and stated as a prompt lever in `agent.js`'s "WEB FACTS" block: *search when you need a concrete fact to answer well, not when you feel unsure* — "I'm not certain" is not a trigger, general abstraction is not a trigger. An agent-initiated search the founder didn't ask for is *more* at risk of being read as evidence, hence the hard marker and the visual distinctness from a research pass.
- **Broad, founder-pushed (`find` `mode:"broad"`, and every slash / `@Mill find`).** When the founder explicitly asks to look something up or dig in: **5 sub-queries × 8 results** — breadth, not depth (`search_depth` stays `"basic"`, never a research pass). **Two generations**: (1) a full report from the search corpus, written at whatever length the evidence supports (`REPORT_PROMPT`, `max_tokens` 8000), goes to `ideas/<id>/find/<stamp>.md`; (2) a *separate* short summary **of that report** (`RUNDOWN_PROMPT`, ~180 words) is the Slack message. The Slack rundown is never the source of the file — that was the bug that made the attached report a ~425-word summary instead of a ~1500-word report. The file (+ query list + every scanned source) is committed and attached to the thread (`files.uploadV2` — needs the `files:write` scope; degrades to "saved to `<path>`" without it). A one-line entry is appended to `ideas/<id>/find/index.md`, which `readOriginContext` loads into every thread of the project — so a report run in Brainstorm is referenceable from Research, Prototype, etc. It is still surface search, not evidence: the file header, the index, and the context block all say so, and `find/` is a **separate directory from `docs/`** precisely so it never reaches `/test`'s `DOC_PATH` or the audit as a document. In a bare `#chats` (no project) there's nowhere to store a report, so `mode:"broad"` there posts the old capped inline message.
...
### D-54 · One thread per project with modes; personas defined by refusal; audit reads a compressed reference doc

**Decision.** `docs/build-prompt-modes.md` (five changes, built in the order Change 5, 2, 1, 3, 4, then migration) restructures projects:

- **Supersedes D-47's five stage threads.** A project is one Slack thread. `state.json` carries `mode` (`brainstorm | product | engineering | proto | audit`), persisted, displayed on every switch, changed by `@Mill mode <name>` or the `mode_switch_<mode>` button row. Every project starts in brainstorm — not skippable as an entry point, since nothing downstream has anything to feed from otherwise. Modes are sequential in *dependency* (each persona's input document is the previous mode's output) but not in *gating* — switching to any mode is always allowed; a missing input document is an offer (below), never a hard stop.
- **Four personas, defined by refusal, not tone** (`personas.js`): Co-founder (brainstorm — refuses an unfalsifiable claim), PM (product — refuses a feature with no user/JTBD/metric, and refuses to prescribe implementation), Engineer (engineering — refuses a design with no failure modes/cost estimate, and refuses to reopen product decisions), Builder (proto — refuses to build without an engineering spec; unchanged autonomous build loop, D-53, and the five-touch cap). Every refusal names its unblock (`REFUSAL:`/`UNBLOCK:` contract), machine-parseable the same way `/attack`'s `TOO_VAGUE:` already was.
- **The feeding rule** (`mode-docs.js`): a persona sees its own input document plus the live thread, nothing further upstream. Document lengths differ by purpose — research knowledge base and audit reference uncapped, product spec 2-3 pages, engineering spec as long as the design requires (the one document that earns detail). A document attached to Slack is always read back from disk after writing, never re-rendered from the in-thread summary — a bug from the `/find` report attachment (D-53) that this generalizes the fix for.
- **Missing/stale documents** (`mode-docflow.js`, `doc-meta.js`): entering a mode with no input document offers generate-or-switch (generates via the *missing* document's own persona, e.g. entering engineering with no product spec generates the product spec via PM from the research base — never an engineering shortcut). An upstream change is diffed against a stored generation snapshot and classified cosmetic / additive / contradictory; additive or contradictory offers section-scoped regeneration, verified to leave untouched sections byte-for-byte unchanged. Never a silent overwrite of what a founder wrote.
- **Amends D-28.** The auditor still never reads the raw thread. It now also reads `ideas/<id>/audit-reference.md` — append-only, timestamped, compressed by `flash-fast` every 5 turns across every mode (`audit-reference.js`, hooked into `chat-session.js`'s `addTurn`). The compressor is framed as attributed transcription, not bias removal, tagging every claim `[SOURCE: research-<stamp>]` / `[FOUNDER BELIEF]` / `[REPORTED SPEECH]`, designed against two failure modes: over-stripping (an argument containing a fact must survive; only the conviction goes) and laundering (speculation — "we think X" — must never be rewritten as an observed finding; D-33/D-49 exist to block exactly this arriving by a new route). Every entry records which mode produced it, cites specific research stamps, carries a running header of every research pass, and flags claims that contradict or ignore an existing report. Append-only — a later contradiction is appended, not resolved.
- **Audit is entered, not triggered.** `/audit`'s old hard refusal on missing/stub research is gone — it now rules on whatever evidence exists, including none (`evidence_basis: "none"` is a valid, expected answer). C-07's `enforceEvidenceGate` is unaffected and still fires unconditionally: `none`/`web-only`/`field-intent` still cap at `narrow` regardless of the refusal removal.
- **Every new interactive element (`mode_switch_<mode>`, `generate_doc`, `regenerate_stale`, `stale_ack`) is built on `button-resolve.js` from the start** — a tapped button strips its actions block and shows the outcome in place, guarded against double-taps by an in-process claim set. This generalizes the fix for the pre-existing bug where the weekly profile diff's approve/reject posted a separate confirmation and left both buttons live and tappable (`profile_diff_approve/reject`, `promote_chat`, `proto_mount/dismount/extend/takeover`, `stale_kill` were all retrofitted onto the same helper).

**Found live, fixed during the build (not hypothetical).** Slack requires `action_id` to be unique *within a block* — the first real post of the five-mode button row (`invalid_blocks`, surfaced migrating `f05e`) had all five buttons sharing one `action_id`. Every mock-client verification passed because nothing validated against the real API; only a genuine `chat.postMessage` call caught it. Fixed to `mode_switch_<mode>` per button, routed in `index.js` via a single regex handler (`/^mode_switch/`), mode read from `value` not `action_id`. Recorded because it's the same shape of lesson D-51 already logged once (verify platform assumptions against the real API, not the mock) — a second occurrence, in a different corner of the same platform.

**Migration.** `ideas/f05e/` (the only idea with real content at migration time — 48 real brainstorm-thread turns, four other stage threads holding nothing but their own anchor line) was consolidated by repointing `threads.project` at the *existing* brainstorm thread rather than reposting its content — zero-loss and exact, since the real Slack authorship and timestamps stay where Slack already has them. `audit-reference.md` was generated retroactively from that real history through the same compressor Change 4 uses live. Filesystem backup taken before any write; `origin-chat.md`, `idea.md`, `find/`, and `state.json`'s other fields verified byte-identical after.

**The pre-proto audit suggestion** (Change 4's "one suggestion before proto") is built: entering proto mode with no audit verdict on file posts an ignorable suggestion once per idea, recorded in `state.json` (`audit_suggested`) at post time — not on tap — so it never repeats regardless of whether the founder acts on it. It cannot block or undo the mode switch, which has already happened by the time it posts. This is the only prompt to audit anywhere in the system now that audit is entered rather than triggered.

**Amendment — the compressor's four-tag taxonomy, and a bug that made its contradiction check unimplementable.**

Verifying the compressor against f05e's real migrated doc turned up three things. Grouped here because they are one lesson about the same prompt.

**The tag taxonomy was incomplete, and the gap was being papered over by luck.** The original three tags were `[SOURCE: research-<stamp>]`, `[FOUNDER BELIEF]` and `[REPORTED SPEECH]`. Surface search fit none of them, so when a `/find` result appeared in an f05e thread the model improvised `[FOUNDER BELIEF] *(Note: Surfaced via automated surface web search…)*` — the right call, but one the prompt never asked for and therefore could not rely on. A future run could as reasonably have read a Tavily snippet as "a retrieved report" and tagged it `[SOURCE: research-…]`, laundering surface search into research evidence at the gate — the exact thing `find/` being a separate directory from `docs/` exists to prevent (D-53). Worse, `[REPORTED SPEECH]` fired twice on *"based on a few conversations"* — no named person, no current spend, no price — and the audit prompt named `[FOUNDER BELIEF]` (excluded) and `[SOURCE: …]` (counts) while saying **nothing** about `[REPORTED SPEECH]`, leaving it in an undefined bucket that reads like field evidence. That is D-49 inverted: grading happens once, at the gate, from raw notes, and a compressed paraphrase is precisely the founder's characterisation D-49 keeps away from the auditor.

Now four tags, each non-evidence tag carrying its disclaimer *inside the tag* (`[SURFACE SEARCH — not evidence]`, `[RELAYED — not field evidence]`) so a reader with no other instructions still cannot mistake one for evidence, plus "if a claim could take more than one tag, use the weaker one." `commands/audit.js` names all four with an explicit counts / never-counts split, and states that a `[RELAYED …]` item can never by itself justify any `field-*` grade.

**The CONTRADICTS check could not have worked.** The prompt asked the model to check each claim against each report's *findings*, but the only research context passed was `researchHeaderText()` — stamp, assumption, `evidence_basis`. Titles, not contents. A founder claiming brands currently pay >₹25k against a report finding no brand pays more than ₹12k could never be flagged, because ₹12k was never in the prompt. `researchFindingsText()` now passes capped report bodies (2k chars/report, 8k total, env-tunable) as a **stable system message** ahead of the volatile turns, so it lands in the cached prefix rather than being re-billed in full on every compression.

**The process lesson, which is the durable one.** The first version of the compressor test asserted only that CONTRADICTS *should* fire on a willingness-to-pay claim against a current-price finding. It didn't fire, and the response was to tighten the prompt — twice — to make it. Both tightenings were wrong-headed: the model was right to abstain (a claim about what someone *would* pay is not contradicted by what something *currently* costs — the same intent-vs-behaviour line D-49 already draws), and it was in any case *blind*, since the findings never reached it. A one-directional test on the system's most sensitive prompt is how that prompt gets tuned toward a wrong answer while every run still looks green. `_c4compressor.js` is now bidirectional by construction: every CONTRADICTS case comes as a pair, one that must fire and one that must not, so the guard is proven to be doing work rather than the model merely being uninformed. Verified stable across repeated runs (17/17).

**Superseded by the chats-with-modes model (recorded because the amendment below still reads as current).** The single pinned "current state" card at channel root, and the `<state> · <mode> · <assumption>` channel topic described in the amendment that follows, are **gone**. A project now holds many chats, each chat's card *is* its thread's root message (`chat-card.js`), and the pin follows the last active chat. A single channel topic cannot represent "the" mode once one channel hosts several chats in different modes, so the topic is set once at creation, with the assumption, and not maintained. `state-card.js` remains only as a compatibility shim so the eight call sites that call `upsertStateCard(client, id, {latestTs})` keep working — it routes to the chat layer. `nextStep` and the lifecycle rendering are gone with it: they encoded D-41's linear pipeline, which modes replaced. The amendment below is kept as the record of what those flaws were and why the card was rebuilt, not as a description of the running system.

**Amendment — the state card's flaws, and one live row at a time.**

The pinned card is the one message a returning founder reads, so its defects are disproportionately costly. Five, found by inspecting the running f05e channel rather than the code:

- **It was littering its own channel.** `setTopic` ran on every `upsertStateCard`, and Slack posts a `channel_topic` system message on each successful call — five had accumulated in f05e. Now set only when the value actually changed, with the last value tracked in `state.json` (`topic_text`) so the guard costs no API call.
- **The topic omitted the mode.** It is the only indicator visible without opening anything, and under this decision the mode determines which persona answers you. Now `<state> · <mode> · <assumption>`.
- **`nextStep()` was mode-blind**, switching on lifecycle state alone — a founder in engineering mode at state `open` was told to "run `@Mill test`", a different mode's job. It now reports the current mode's unmet precondition first, since that is what is actually blocking them.
- **The card couldn't show the document chain** the whole mode model turns on. It now carries `*Docs:* ✓ research KB · — product spec · — engineering spec`.
- **The pin was attempted only at first post.** A pin that failed once (transient error, or the scope granted later) stayed unpinned forever, because the card exists thereafter and the repost branch never runs again. Re-asserted on the update path; `already_pinned` is the normal case and is ignored.

**A rendering bug in `mrkdwn.js`, visible on the card as `*Idea \`f05e\`• · *open*`.** `eachOutsideInlineCode()` fragments a line at backticks, so the closing `*` of a bold span began its own segment — and the bullet rule, anchored on `^`, treated that *segment* start as a *line* start and rewrote it to `•`. A segment boundary is not a line boundary. Line-level transforms now run on intact lines before inline code is split out, which is safe because an inline code span cannot contain a newline.

**One live mode row at a time.** A button-path switch resolved the tapped row via `button-resolve`, but a command-path switch (`@Mill mode engineering`) posted a new banner and left every earlier row live, still marking the old mode as current — so a thread accumulated rows visibly disagreeing about where the project was. `switchMode` now retires the previous banner (tracked as `mode_banner_ts`), skipping the message a tap is already resolving so it never double-appends an outcome line.

**f05e brought onto the new format.** The founders use that project as the demo other founders are shown, so leaving it in the old shape would have taught the superseded structure. Its brainstorm mode had no `research-kb.md` despite 28 origin-chat turns and 42 substantive thread turns of real work — meaning a founder switching to product planning would have been told the knowledge base didn't exist. Generated it (2,164 words) through the brainstorm persona from that actual history, marked auto-generated with its provenance and an explicit note that no research pass has run, so nothing in it is evidence. The project thread root was rewritten from the old Brainstorm stage-anchor text to the project-anchor header, and 20 obsolete messages were deleted with the founder's approval — bot error artifacts, the dead five-thread furniture and its retire notes, and the `channel_topic` spam — leaving one anchor, one thread and one pinned card. Founder-authored messages were left alone, including duplicates caused by bot failures.

**Revisit when:** the findings cap (2k/8k chars) starts truncating away the numbers the contradiction check needs — at which point precompute a short findings digest per report at `/test` time rather than raising the cap, since compression runs every 5 turns and the cost is paid on each. Otherwise never, without an explicit decision to reverse the single-thread model — consistent with D-47's own revisit clause, which this entry supersedes.

---

### D-55 · Modes reach conversation; tools suppress refusals; the mind features feed from projects

**Context.** D-54 built the personas but wired them only into document generation. `runPersonaTurn` was called from `generateDraft` and the missing/stale flow, never from `chat-turn.js`/`agent.js`. So switching a chat to engineering changed the card's label, which document was fed, and the missing/stale checks — but not who replied. Every reply came from one generic prompt. The refusals that define the personas ("personas must be distinguishable by what they will not accept") could not fire where they actually bite.

**Decision.** The chat's mode selects the persona for ordinary conversation, layered onto the agent loop so tools remain available (D-53's single loop is unchanged).

**Three things had to be fixed for that to mean anything, and each was found by measurement rather than reasoning.**

**1. The feeding rule applied to documents but not to conversation.** An engineering-mode chat had neither the product spec nor its own engineering spec in context, so the engineer replied *"I can't do this without stated failure modes"* about a design whose failure modes were written in its own spec three lines away. `buildContextMessages` now applies the same rule `runPersonaTurn` does: the previous stage's document plus this mode's own current document, nothing further upstream.

**2. Identity was asserted three times, and the persona lost.** `agent.js`'s `SYSTEM_PROMPT` opened *"You are Mill, a founder's thinking partner"* and `CONTEXT_SYSTEM_PROMPT` opened *"You are Mill, a thinking partner… **Engage with what they actually said**"* — the latter close to the inverse of "refuse", and its *"nothing here is stored"* claim simply false inside a project. Identity is now single-sourced from `personas.js`; those two blocks carry tool mechanics and style only.

**3. THE REFUSAL GATE — offering tools suppresses refusal.** With a byte-identical message list, the persona refuses correctly when called **without** tools (citing the product spec by name, with the right unblock) and emits a bare tool call with **no prose at all** when the same messages are sent **with** tools. Presenting a tool set pulls the model toward acting rather than declining. This is not a prompt problem and cannot be fixed by emphasis — four rounds of wording and ordering changes moved it not at all. Non-brainstorm modes therefore run a **no-tools veto pass first**: one cheap `flash-fast` call asking only whether this role refuses. A refusal is posted and *replaces* the agent call; anything else falls through to the normal loop. Brainstorm — the common case — is untouched and costs nothing extra. A failed veto never blocks the turn.

**The mind features.**

- **`profile.md` and recent captures reach brainstorm only.** The co-founder attacks from the founder's blind spot, which is what D-26 built the profile for. The PM, engineer and builder judge a spec on its own merits; feeding them "this founder overrates distribution" would let the profile shape durable artifacts rather than critique. The auditor never sees either (D-28) and never reaches this path.
- **Project chats feed `minds/` captures again.** They were marked `promoted` and skipped by the nightly drain, so a founder doing their thinking inside projects produced *no captures at all* — `/themes` saw nothing and the weekly profile diff (D-30) read an empty week, while project chats are precisely where the thinking now happens. Captures are the raw substrate D-26/D-42 assume.
- **One chat concept.** A `#chats` session is a project-less **brainstorm** chat: same persona, mode fixed to brainstorm because no documents exist without a project. `sessionMode()` is the single answer to "what mode is this chat in", for both tiers.
...
**Amendment — partial acceptance, and a third veto outcome.** The gate was all-or-nothing, and that lost founder work. A founder asked the PM to modify the spec with two things: an agency-integration screen (no success metric) and a brand reporting screen (fully specified). The veto refused, the veto *replaces* the turn, so the requested `save` never ran and **both** were lost. Founders' choice of three options: write what passes, refuse the rest by name, in one message.

- **The veto now has three outcomes**, not two: `PROCEED`, `REFUSAL:`/`UNBLOCK:` (nothing in the request this role can act on), and `PARTIAL` (the founder asked for a document write and some of it meets the bar). `PARTIAL` falls through to the loop so `save` runs. It is a third answer from the same call, not a second classifier — D-53 removed the classifier and this does not bring it back.
- **The write applies the bar per item.** Items that pass go in; items that don't are left out — never as a weakened placeholder — and reported after the document behind a `---NOT-INCORPORATED---` trailer, one `REFUSAL:`/`UNBLOCK:` pair each. A trailer rather than a JSON envelope for D-51's reason: the document is the payload, and escaping markdown into a JSON string field is a reliability problem.
- **Scoped to `product`, `engineering` and `deck` (`PARTIAL_ACCEPTANCE`), deliberately not `brainstorm`.** Found by a failing test rather than by reasoning: applying it everywhere started excluding founder claims from `research-kb.md`. That document's job is to record what was discussed so downstream modes have the context and the audit chain can see the founder's actual beliefs; filtering it defeats its purpose. "Refuse an unfalsifiable claim" is something the co-founder says in conversation, not a filter on the knowledge base.

**Amendment — two claims the thread made that the system's own state contradicted.** Both found in one live turn, and they are the same defect: a founder cannot distinguish a real result from a claimed one, so a wrong claim is worse than silence.

1. **"I couldn't get a reply out for that one. Nothing was saved."** — posted while the turn was still working; the real answer arrived 29 seconds later. `runTurn`'s `finally` guarantees the placeholder always resolves, but a command that posts its OWN output returns without settling, so the guard fired on every successful tool turn. The guarantee was right; what was missing is that a tool-owned response *is* an answer. `markAnswered()` now records that, and the `finally` stays for the case it was built for (the loop exhausting with nothing said).

2. **"Updated *Product spec* · 1038 → 1038 words"**, with a summary of the document, over a file that had not changed at all — because everything the founder asked for had been deferred to the question below it. A byte-identical document now reports *"No change to Product spec · still N words"*, says *"The document is unchanged"* instead of re-summarising what was already there, and tells the founder *"Nothing went in yet — answer the question below and I'll fold it in."*

**The recurring test trap, recorded because it has now bitten three times in one build.** `doc-sync.js`, `agent.js` and `tools.js` all destructure their dependencies at require time (`const { runPersonaTurn } = require("./mode-docs")`), so a stub installed *after* the module loads is silently ignored — the test then exercises the real path and passes or fails for the wrong reason. Twice this produced a green test that was proving nothing. The fix that survives it is an indirection installed *before* the dependent module loads, not a late reassignment.

**Amendment — the refusal gate runs AFTER the loop, not before it.** D-55's finding is untouched: with an identical message list the persona refuses correctly without tools and emits a bare tool call with them, so the check still runs with **no tools**. What was wrong was *when*.

Running it first made it decide something it cannot see: **whether the turn is a document write.** A founder asking to modify the spec had the whole turn replaced by a refusal, so the `save` — which now asks per item for what is missing rather than refusing over it — never ran. Worse, because the check reads the thread, two earlier refusals for the same message biased it toward a third: the same momentum problem D-52 recorded for command routing, arriving by a new route. A `PARTIAL` outcome was tried first and is **removed** — it was a patch on the ordering, and it failed live for exactly the reason the ordering was wrong.

So the loop decides first. A tool call is owned by its command, which enforces its own gates (a save asks or refuses per item). Only when the turn is about to answer in **prose** — the case the gate was actually built for — is the role asked whether it refuses. The check is also told to judge the latest message only, since an earlier refusal in the thread is not a reason to refuse again. Cheaper too: a tool turn now makes one model call instead of two, and brainstorm still makes none.

**Amendment — ask, don't refuse, when the shortfall is missing information.** Partial acceptance stopped one bad item sinking a whole write, but a refusal is still a dead end the founder has to resolve in prose — *"name the user, state the job, give me a metric"* — when the persona has the research base, the current document and the conversation in front of it and can see the plausible answers perfectly well. Making the founder type out what the model could have proposed is work the machine should be doing. Founders' call.

An item that falls short **for want of information** now becomes a **question with the answers already drafted**: two or three concrete recommendations grounded in what the project knows, a *Something else…* custom entry (a modal), and a **Skip** that takes the top recommendation. One tap on a phone. The message says which option skip will use *before* it is tapped, and the resolved message records what was chosen — because a founder who skips has not said "leave it out", they have said "you choose", and what was assumed on their behalf has to stay visible.

**Refusal is not replaced — it is narrowed to the case where no answer would help.** The PM will not put a database schema in a product spec however the founder answers, so that stays a `REFUSAL:`/`UNBLOCK:` naming where it belongs. The persona decides which kind each shortfall is, and the trailer it emits says which.

**The answers re-enter the conversation.** When every question is answered the answers are appended as a founder turn and the write re-runs, so they are visible in the thread and reach `audit-reference.md` the same way anything else the founder said does — not hidden state. Capped at `MILL_DOCQ_MAX_ROUNDS` (2), after which outstanding items become plain exclusions rather than asking forever.

**Two things found by testing, not reasoning.** The PM correctly refused a Postgres schema and then wrote `REFUSAL: …` as the closing line **of the spec itself**, because clause (b) named no delimiter where clause (a) did — so the file gained a refusal and the founder was told nothing was excluded. Fixed in the prompt *and* guarded in code: trailing `REFUSAL:`/`UNBLOCK:` lines are lifted out of any document, whatever the model does with the delimiter. And the question message was posted to the real Slack API before being trusted — mocks accept `invalid_blocks` all day (D-54).

**The trade, stated because founders will feel it:** the PM and engineer are stricter line by line. Anything added to a spec without a named user, a job and a metric produces a question rather than being quietly written in. That is the bar working; if the questions become noise rather than signal, the fix is to soften the persona's bar, not to weaken the asking.
...
**`textMode` is a founder choice per deck, not a constant — corrected after measuring.** This entry originally fixed `preserve` in code on the reasoning that `generate` would put a non-LiteLLM model in the authoring path. A founder asked why not simply use `generate` with a precise prompt, and the honest answer needed a measurement rather than an argument. Both probes are in `ops/BUILD-LOG.md`. `generate` **kept every fact given to it** (Shopify, WMS, 10-digit HTS, de-minimis, ops lead, threshold, shortfall) and produced **step-sequence flow layouts that `preserve` cannot** — restructuring prose into labelled boxes *is* rewriting, so preserve is definitionally incapable of it. It also cost 9 credits against 15. The price is register, not accuracy: it added "seamless", "purpose-built", "automated" — marketing prose nobody wrote. So `preserve` remains the default, because an evidence-bearing deck should say what the founder said, and `generate` is offered as a labelled choice ("Let Gamma design it (rewrites)") for decks where visual structure is the point. Neither mode emits chart or diagram XML parts; generate's "diagram" is Gamma's own layout components. The rule that survives is narrower than the original: textMode must never change *silently*, because that would make the persona decorative with nothing to reveal it.

**Charts, tables and process flows are supported — but they void `preserve`, so instructions are dropped there.** Gamma's structured-content guide describes charts/tables/infographics driven by explicit data plus a natural-language instruction, with no special syntax, and it covers the *generate* API only. Probed in preserve mode with `Q1: $1.2M / Q2: $1.5M / …` and an instruction to chart it: Gamma **did** render the chart (rasterised, not a PPTX chart object — `noImages` was set and two large graphics still appeared), **and replaced the literal figures with prose containing "a 75% increase over the full year"** — a computed claim nobody wrote, on the artifact most likely to be shown to outsiders. So `additionalInstructions` overrides `preserve` and silently voids the guarantee this whole integration rests on. It is therefore **dropped under preserve and passed only in generate**, where rewriting is the declared behaviour and the founder has chosen it. Consequence, stated plainly: charts and "my words exactly" are mutually exclusive. Cost also jumps — 40 credits for two chart slides against ~5/slide for plain text. The deck persona is told to put real figures on the slide, never to state a derived figure the founder did not give, and to tell the founder a chart needs "Let Gamma design it".
...
**Amendment — automatic syncing removed; `save` is a tool instead.** The reasoning above expired rather than being wrong. Automatic syncing existed because `@Mill save` was the only way to write a document and *nothing surfaced it* — so a founder could work for an hour and carry none of it forward. What was actually missing was not automation but reach: the agent had ten tools and **none of them wrote a document**, so a founder in product mode saying *"can you create a product spec"* — the plainest possible phrasing of exactly that operation — got prose, and the file appeared minutes later as a side effect of a turn counter. Two mechanisms wrote one file and the one the founder could address was the one that didn't exist.

`save` is now an action and a tool (project-only, no arguments, writes the current chat's mode document). Asking for it runs it, the same way "attack this" runs `/attack`.

**Only the every-6-turns fold is removed.** It fired mid-work on a turn counter, so a document changed with no founder action to attribute it to.

**The on-mode-switch sync stays, and the distinction is the whole point.** Both were removed at first, on the reasoning that `save` now covers them; the founders corrected it the same day: *"the user won't always remember to ask for a save."* That is right, and the two cases are not alike. A switch is a **boundary the founder just created** — it is itself the signal that the previous stage's conversation is finished — and the write announces itself with a word delta, so there is a cause to point at. More importantly the cost of forgetting is **silent and deferred**: the next persona reads a stale upstream document and answers confidently from it, which is precisely the failure this entry was written to prevent. An explicit action cannot substitute for automation whose absence is invisible at the moment it matters.

**Also not removed:** the missing-input and stale-input offers, which are button-gated, so a founder approves each of those writes — the D-30 principle the original entry invoked.

**One prompt, not two.** `saveModeDocument` had carried its own directive — *"rewrite it as one coherent document"* — while `doc-sync.js` carried the stricter per-section UNCHANGED / APPEND / REVISE decision for the same file. The strict one exists because a blank-page rewrite is what makes a model silently drop facts (proven when a save wiped a DGFT trade notice, three competitor names and the unit economics). The weaker prompt sat on the path a founder actually reaches. Save now *is* the sync, invoked explicitly, so there is one document-writing path in the system.

**Verified on the founder's own question** — "if I discuss and then ask it to modify the spec, does it modify, overwrite, or do nothing?" A save replaces the file, so at the filesystem level modify and overwrite are the same operation; the only thing that makes it a modification is whether the prior document survives the model call. `_c21modify.js` proves it bidirectionally against a spec stuffed with specifics: the two requested changes land, and the DGFT notice, both named competitors, the MoCRA and 10-digit HTS details and the 48-hour metric all survive. The superseded price appears only under `## Changed in this update`, never in a live section. An immediate second save reports *"nothing new to add"* rather than rewriting.

**Revisit when:** founders find themselves asking to save so routinely that it becomes noise — at which point the answer is a prompt to save on mode switch, not a silent write.
